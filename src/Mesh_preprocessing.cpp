...
{	
	...
	surrounding_set.clear();
        ...
        for(int i = 0; i < nIndex; i++)
        {
        	for(int j = 0; j < 3; j++)
        	{
        		if((triangles[i].points[0] == triangles[expand_id].points[j])||(triangles[i].points[1] == triangles[expand_id].points[j])
        		||(triangles[i].points[2] == triangles[expand_id].points[j]))  // todo: reduct computational time
        		{
